...
const CustomSingleSelectOption = ({ title, data, option, setOption }) => {
  return (
    <Menu as='div' className='relative inline-block text-left'>
      <Menu.Button>
        <div className='bg-white px-4 py-2 rounded-md space-y-2 ring-1 ring-blue-light ring-opacity-10 w-24'>
          <p className='text-xs font-medium text-gray-600'>{title}</p>
          <div className='flex justify-between'>
            <p className='text-sm font-medium text-black'>{option}</p>
        ...
        <Menu.Items className='absolute z-10 w-full mt-2 origin-top-right bg-white rounded-md shadow-sm ring-1 ring-blue-light ring-opacity-30 focus:outline-none px-4 py-2 space-y-1'>
          ...
          <Menu.Item>
            <div className='text-center'>
              <span className='bg-black text-white text-xs cursor-pointer rounded px-2'>
                close
              </span>
            </div>
          </Menu.Item>
